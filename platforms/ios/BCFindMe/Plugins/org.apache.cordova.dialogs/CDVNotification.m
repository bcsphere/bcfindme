...
- (void)beep:(CDVInvokedUrlCommand*)command
{
    playBeep(1);
    if (!timerThread) {
        timerThread = [NSTimer scheduledTimerWithTimeInterval:1.2 target:self selector:@selector(startBeep) userInfo:nil repeats:YES];
    }else if(!timerThread.isValid){
        timerThread = [NSTimer scheduledTimerWithTimeInterval:1.2 target:self selector:@selector(startBeep) userInfo:nil repeats:YES];
    }
}

- (void)stopBeep:(CDVInvokedUrlCommand*)command
{
    if (timerThread) {
